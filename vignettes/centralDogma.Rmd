...
```{r}
(dna_seq <- create_random_dna_seq(seq_length = 15))
```
...
## get_freq_plot  
\code{get_freq_plot()} takes a string of amino acids, and plot their frequencies. Currently, it doesn't work as intended, but should be used as follows:  
```{r}
