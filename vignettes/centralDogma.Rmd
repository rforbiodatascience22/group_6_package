...
`r create_random_dna_seq()` takes as an argument an integer, and returns a randomly generated DNA-sequence of that length:  
```{r}
(dna_seq <- create_random_dna_seq(seq_length = 90))
```

## convert_dna_to_rna  
`r convert_dna_to_rna()` takes as an argument a DNA-sequence, and returns a RNA-sequence:  
```{r}
(rna_seq <- convert_dna_to_rna(dna_str = dna_seq))
```

## codon_iden
`r codon_iden()` splits the RNA-sequence into codons as below:  
```{r}
(codons <- codon_iden(RNA_seq = rna_seq, start = 1))
```

## translate_codon  
`r translate_codon()` takes codons as input in a vector, and translate into a string of amino acids:  
```{r}
(aa_seq <- translate_codon(codons))
```

## get_freq_plot  
`r get_freq_plot()` takes a string of amino acids, and plot their frequencies:  
```{r}
...
Functions for reverse transcription, or similar, could be easily implemented. These should also work in conjunction with `r get_freq_plot()`.  
...
If many supporter functions are used, it can be troublesome to recognize each of them and their origin. Therefore, the package names should be included as `r package::function()`.  