...
`create_random_dna_seq()` takes as an argument an integer, and returns a randomly generated DNA-sequence of that length:  
```{r}
(dna_seq <- create_random_dna_seq(seq_length = 60))
```

## convert_dna_to_rna  
`convert_dna_to_rna()` takes as an argument a DNA-sequence, and returns a RNA-sequence:  
```{r}
...
## translate_codon  
`translate_codon()` takes codons as input in a vector, and translate into a string of amino acids:  
```{r}
(aa_seq <- translate_codon(codons))
```

## get_freq_plot  
`get_freq_plot()` takes a string of amino acids, and plot their frequencies:  
```{r}
...
Functions for reverse transcription, or similar, could be easily implemented. These should also work in conjunction with `get_freq_plot()`.  
...
If many supporter functions are used, it can be troublesome to recognize each of them and their origin. Therefore, the package names should be included as `package::function()`.  